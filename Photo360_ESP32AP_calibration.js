...
var run = function () {
  ...
  function wifiConnect() {
	wifi.disconnect();
	wifi.startAP(config.wifiSsid, {authMode:"wpa2", password:config.wifiPassword});
...
///////////////////////////
  ...
  function Stepper() {
    ...
    var stepTimer = setInterval(function () {
      if (accSteps * 3 < stepsLeft && startSpeed < config.speed) {
        startSpeed += (startSpeed/100)*accStep;
  ...
  function TurntableCalibration() {
	if (!startFlag) {
      return;
    }
	//stop calibration after 2 min, if no user stop occured
	var CalibrationTimer = setTimeout(function () {
		Stop();
    }, 120000);
